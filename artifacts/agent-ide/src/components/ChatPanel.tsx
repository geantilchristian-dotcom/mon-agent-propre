import React, { useState, useRef, useEffect } from "react";
  ...
  import { Loader2, Send, RotateCcw, Bot, User, FileCode, Paperclip, X, Check, Zap } from "lucide-react";
  ...
  export function ChatPanel({ currentPath, onApplyCode }: ChatPanelProps) {
    ...
    // Sync model status with mutation state
    useEffect(() => {
      if (sendMutation.isPending) {
        setModelStatus("thinking");
      } else if (sendMutation.isError) {
        setModelStatus("error");
      } else if (sendMutation.isSuccess) {
        setModelStatus("ok");
      } else {
        setModelStatus("idle");
      }
    ...
    const statusDot: Record<ModelStatus, { color: string; label: string; pulse: boolean }> = {
      ...
      error:    { color: "bg-red-400",              label: "Erreur modèle",   pulse: false },
    };
      ...
      <div className="flex flex-col h-full bg-sidebar/50">
        ...
        <div className="px-3 py-1.5 border-t border-border bg-muted/10 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-medium text-muted-foreground">Gemini 2.0 Flash</span>
          </div>
          <div className="flex items-center gap-1.5">
